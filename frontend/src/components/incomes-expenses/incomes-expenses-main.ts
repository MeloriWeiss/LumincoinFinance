...
export class IncomesExpensesMain {
    constructor() {
        ...
        document.getElementById('delete-operation').onclick = () => this.deleteOperation();
    ...
    buildTable(operations) {
        ...
        if (this.rowIdElements) {
            for (let operation of this.rowIdElements) {
                operation.onclick = () => {
                    this.currentOperation = operation.getAttribute('data-id');
                }
            }
        }
    }
    ...
    async deleteOperation() {
        let result = await HttpUtils.request('/operations/' + this.currentOperation, 'DELETE');
        if (result) {
            for (let i = 0; i < this.rowIdElements.length; i++) {
                if (this.rowIdElements[i].getAttribute('data-id') === this.currentOperation) {
